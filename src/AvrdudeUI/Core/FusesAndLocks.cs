...
{
    // Credits: Simone Chifari (Fuse selector)
    public class FusesList
    {
        ...
        public class FuseBitNames
        {
            ...
            public FuseBitNames() : this(new FuseBitNamesXML.McuBitsXML()) { }
        ...
        private void Load()
        {
            ...
            try
            {
                var path = Path.Combine(AssemblyData.directory, FILE_BITS);
                bits = new XmlFile<FuseBitNamesXML>(path, isFullPath: true).Read();
            }
            catch (Exception ex)
            {
